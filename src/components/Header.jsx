...
const Header = ({ toggleSideBar }) => {
  ...
  const handleSearch = (e) => {
    ...
    const result = AllData.filter((item) => item.title.toLowerCase().includes(query));
  ...
  {searchQuery && (
    <div class="absolute z-50 bg-white w-full shadow-lg rounded-md left-1/2 transform -translate-x-1/2 top-full mt-1 flex flex-col">
      {searchResult.map((item, index) => (
        <div className="px-2 py-1 cursor-pointer" key={index} onClick={() => handleNavigate(item.link)}>
          <p className="hover:bg-primary hover:text-white p-2 rounded-lg">{item.title}</p>
        </div>
      ))}
    </div>
  )}
