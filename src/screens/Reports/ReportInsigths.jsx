...
const ReportInsights = ({ filteredData, generateData }) => {
  ...
  const getInsights = () => {
    ...
    let startDate = generateData.startDate
    ...
    if (!startDate && filteredData.length > 0) {
      ...
      startDate = new Date(
        Math.min(...filteredData.map((item) => new Date(item.timestamp)))
      );
    }
    ...
    if (!endDate && filteredData.length > 0) {
      ...
      endDate = new Date(
        Math.max(...filteredData.map((item) => new Date(item.timestamp)))
      );
    }
    ...
    if (filteredData.length >= 2) {
      ...
      if (sortedDates.length >= 2) {
        ...
        const firstHalfAvg =
          sortedDates
            .slice(0, midpoint)
            .reduce((sum, date) => sum + dateGroups[date], 0) / midpoint;
        const secondHalfAvg =
          sortedDates
            .slice(midpoint)
            .reduce((sum, date) => sum + dateGroups[date], 0) /
          (sortedDates.length - midpoint);
    ...
    filteredData.forEach((item) => {
      if (item.location && item.location.geoCodeLocation) {
        locationCounts[item.location.geoCodeLocation] =
          (locationCounts[item.location.geoCodeLocation] || 0) + 1;
      }
    });
    ...
    const comparisonToAverage = (
      (averagePerDay / baselineAverage) * 100 -
      100
    ).toFixed(0);
      ...
      <div className="text-sm space-y-2 text-gray-600">
        ...
        <p>
          <span className="font-medium">
            Total <span className="font-bold">{emergencyType}</span>{" "}
            Emergencies:
          </span>{" "}
          {insights.totalCount}
        ...
        <p>
          ...
          {insights.averagePerDay} incidents per day
        </p>
        {insights.comparisonToAverage !== 0 && (
          <p>
            ...
            <span
              className={
                Number(insights.comparisonToAverage) > 0
                  ? "text-red-500"
                  : "text-green-500"
              }
            >
              {" "}
              {Math.abs(insights.comparisonToAverage)}%{" "}
              {Number(insights.comparisonToAverage) > 0 ? "higher" : "lower"}{" "}
              than usual
            </span>
          </p>
        )}
