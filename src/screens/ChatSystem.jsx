...
const MessageBubble = ({ text, isOwn, timestamp }) => {
  ...
  return (
    <div className={`flex ${isOwn ? "justify-end" : "justify-start"} mb-4 px-4`}>
      <div className={`flex flex-col max-w-xs lg:max-w-lg text-wrap ${isOwn ? 'items-end' : 'items-start'}`}>
        <div
        ...
        >
         <p className="text-sm leading-relaxed break-words overflow-hidden max-w-lg">{text}</p>
...
const ChatList = ({ onSelect, selectedUser }) => {
  ...
  const countUnreadMessages = (messages, lastReadTimestamp, otherUserId) => {
    ...
    return messageArray.filter(msg => 
      ...
      msg.timestamp > (lastReadTimestamp || 0)
    ).length;
  };
  ...
  useEffect(() => {
    ...
    const unsubscribeChats = onValue(userChatsRef, async (snapshot) => {
      ...
      userIds.forEach(userId => {
        ...
        newUnreadCounts[userId] = countUnreadMessages(messages, lastRead, userId);
      });
  ...
  const handleUserSelect = async (user) => {
    ...
    if (unreadCounts[user.uid] > 0) {
      const lastReadRef = ref(database, `lastRead/${currentUser.uid}/${user.uid}`);
            ...
            return (
              <div
                ...
                className={`p-4 cursor-pointer hover:bg-gray-50 transition-colors duration-150 border-b border-gray-100 ${
                  selectedUser?.uid === user.uid ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
...
const ChatWindow = ({ selectedUser, setSelectedUser }) => {
  ...
  useEffect(() => {
    ...
    const markAsRead = async () => {
      const lastReadRef = ref(database, `lastRead/${user.uid}/${selectedUser.uid}`);
      await set(lastReadRef, Date.now());
    };
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-white">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <h3 className="font-semibold text-gray-500 text-xs">{selectedUser.customId}</h3>
          </div>
        </div>
      </div>
        ...
        <div className="py-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full py-12 text-gray-500">
              ...
              <h3 className="font-medium text-gray-900 mb-1">{selectedUser.name}</h3>
